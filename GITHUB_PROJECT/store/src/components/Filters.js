...
const Filters = () => {
  const {
    filters:{
    text,category,shipping,color,min_price, max_price, price, company
   },
   all_products,
   clearFilters,
   updateFilters
              ...
              <div>
                {
                  categories.map((c, index)=>{
                    return <button key={index} name="category" onClick={updateFilters} type="button" className={`${category===c.toLowerCase()?'active':null}`}>{c}</button>
                  })
                }
              </div>
            </div>
                    ...
                    <div className='colors'>
                      {
                        colors.map((c, index)=>{
                          if(c==='all'){
                            return (
                              <button key={index} name='color' onClick={updateFilters}
